...
%             end
        ...
        function parse_text(obj)
            ...
            %%%%%%%%%%%%%%%%%
            % Parse receivedNewScenarioCls Messages:
            %%%%%%%%%%%%%%%%%
            receivedNewScenarioClsMsgId = contains(obj.textLines,'Received new scenario command:Cls:');
            obj.isRead(receivedNewScenarioClsMsgId) = true;
            if any(receivedNewScenarioClsMsgId)
                receivedNewScenarioClsLines = obj.textLines(receivedNewScenarioClsMsgId)';
                obj.receivedNewScenarioClsMsg.time = datetime(extractBefore(receivedNewScenarioClsLines,24),'InputFormat','yyyy-MM-dd HH:mm:ss,SSS');
                obj.receivedNewScenarioClsMsg.value = extractAfter(receivedNewScenarioClsLines,'Received new scenario command:Cls:');
            end      
            
            %%%%%%%%%%%%%%%%%
            % Parse newClassDecision Messages:
            %%%%%%%%%%%%%%%%%
            newClassDecisionMsgId = contains(obj.textLines,'New Class Decision:');
            obj.isRead(newClassDecisionMsgId) = true;
            if any(newClassDecisionMsgId)
                newClassDecisionLines = obj.textLines(newClassDecisionMsgId)';
                obj.newClassDecisionMsg.time = datetime(extractBefore(newClassDecisionLines,24),'InputFormat','yyyy-MM-dd HH:mm:ss,SSS');
                obj.newClassDecisionMsg.value = extractAfter(newClassDecisionLines,'New Class Decision:');
            end  
