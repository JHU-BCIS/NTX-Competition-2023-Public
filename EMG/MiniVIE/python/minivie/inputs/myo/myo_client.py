...
class MyoUdp(SignalInput):
    ...
    def parse_messages(self, data):
        ...
        if len(data) == 48:  # NOTE: This is the packet size for MyoUdp.exe
            ...
            self.__quat = np.array(unscaled[0:4], np.float32) / MYOHW_ORIENTATION_SCALE
            self.__accel = np.array(unscaled[4:7], np.float32) / MYOHW_ACCELEROMETER_SCALE
            self.__gyro = np.array(unscaled[7:10], np.float32) / MYOHW_GYROSCOPE_SCALE
