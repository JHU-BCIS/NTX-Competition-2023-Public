...
class FeatureExtract(object):
    ...
    """
    def __init__(self, data_scale_factor=1.0, num_data_samples=None):
        self.input_source = 0
    ...
    def get_features(self, data_input):
        ...
        if data_input is None:
            ...
            f = np.squeeze(self.feature_extract(data * self.data_scale_factor))

            # TODO: The imu functions below have the potential to be very slow
            imu = None
            rot_mat = None
...
def test_feature_extract():
    ...
    for cls in features.EMGFeatures.__subclasses__():
        ...
        try:
            fe = FeatureExtract()
            feature = cls(incremental=True, window_size=num_samples, window_slide=window_slide_samples,
            ...
            print('*************')
            print(
                f'Feature Class: {cls.__name__}Inc  Feature Name: {feature.get_name()}  Time to complete: {py_time:.5f})')
