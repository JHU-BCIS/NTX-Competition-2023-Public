...
class MplScenario(object):
    ...
    def run(self, loop_counter=float('inf')):
        """
        ...
        while self.loop_counter < loop_counter:
            try:
    ...
    def update(self):
        ...
        if True:
            ...
            packer = struct.Struct('64f') # 16 ch times 4 features
            #packer = struct.Struct('32f')
            packed_data = packer.pack(*values)
            self.DebugSock.sendto(packed_data, ('127.0.0.1', 23456))

        # TODO: Resolve CPCH returning repeated values.  This may be due to the thread not updating buffer fast enough
        # These repeated values don't happen in the live_plot_cpch.py function
        # if (f == self.last_features).all():
        ...
        if self.auto_save and self.add_data_last and not self.add_data:
            self.TrainingData.delete()  # Let's delete, so we have a clean file to write to
    ...
    def update_feedback(self):
        # Check user_config to see if feedback algorithm should run
        ...
        # TODO: Create App control for enabling feedback

        # Get percepts for commanding tactors
        p = self.DataSink.get_percepts()

        if p is not None:
            # TODO: Map joints to specific motors
            torque_val = p['jointPercepts']['torque'][mpl.JointEnum.INDEX_MCP]

            # Get torque thresholds for feedback
            if torque_val < self.index_mcp_limits[0]:
                # Torque is below threshold
                # Turn off feedback
                self.feedback_state = False
                duration = 0
                intensity = 0
            elif self.index_mcp_limits[0] <= torque_val <= self.index_mcp_limits[1]:
                # Torque is within threshold range
                # Turn on feedback
                self.feedback_state = True
                duration = 1
                intensity = int(((torque_val - self.index_mcp_limits[0]) / (
                    self.index_mcp_limits[1] - self.index_mcp_limits[0]) * (0xff-0x20)) + 0x20)
            elif torque_val > self.index_mcp_limits[1]:
                # Torque is above maximum threshold
                # Turn on max feedback
                self.feedback_state = True
                duration = 1
                intensity = 0xff
            ...
            if self.feedback_state:
                ...
                logging.info(f'Feedback state on ({self.feedback_counter}): {duration}, {intensity}, {motor_mask}')
            else:
                self.feedback_counter = 0

            if self.feedback_counter >= self.feedback_timeout:
                duration = 0
                intensity = 0

            # Send feedback message to signal sources
            if duration > 0:
                payload = struct.pack("3B", duration, intensity, motor_mask)
                for s in self.SignalSource:
                    try:
                        s.transport.sock.sendto(payload, (s.transport.remote_addr))
    ...
    def setup(self):
        ...
        elif input_device == 'intan':
            from inputs.intan import intan_client
            num_devices = get_user_config_var('IntanUdpClient.num_devices', 1)
            # Connect first armband
            local_1 = get_user_config_var('IntanUdpClient.local_address_1', '//0.0.0.0:15001')
            remote_1 = get_user_config_var('IntanUdpClient.remote_address_1', '//127.0.0.1:16001')
            self.attach_source(intan_client.IntanUdp(local_addr_str=local_1, remote_addr_str=remote_1))

            if num_devices == 2:
                # Dual Armband Case
                local_2 = get_user_config_var('IntanUdpClient.local_address_2', '//0.0.0.0:15002')
                remote_2 = get_user_config_var('IntanUdpClient.local_address_2', '//127.0.0.1:16002')
                self.attach_source(intan_client.IntanUdp(local_addr_str=local_2, remote_addr_str=remote_2))

        elif input_device == 'alc':
            from inputs import alc_udp_client
            # Connect ALC
            local_1 = get_user_config_var('AlcUdpClient.local_address_1', '//0.0.0.0:14001')
            remote_1 = get_user_config_var('AlcUdpClient.remote_address_1', '//127.0.0.1:14002')
            self.attach_source(alc_udp_client.AlcUdp(local_addr_str=local_1, remote_addr_str=remote_1))
        ...
        elif input_device == 'cpch':
            from inputs import cpch_serial
            port = get_user_config_var('CpchSerial.port', 'COM1')
            diff_analog_channel = get_user_config_var('CpchSerial.diff_analog_channel', 0xFFFF)
            se_analog_channel = get_user_config_var('CpchSerial.se_analog_channel', 0xFFFF)
            src = cpch_serial.CpchSerial(port=port, bioamp_mask=diff_analog_channel, gpi_mask=se_analog_channel)
            src.num_samples = get_user_config_var('CpchSerial.num_samples', 150)
            self.attach_source(src)
        ...
        ################################################
        data_scale_factor = get_user_config_var('Features.scale', 0.01)
        self.FeatureExtract = pattern_rec.feature_extract.FeatureExtract(data_scale_factor)
        ...
        filename = get_user_config_var('MPL.roc_table', 'mpl/#VMPL_ROC.xml')
    ...
    def setup_feedback(self):
        # Feedback variables
        self.enable_feedback = get_user_config_var('MPL.enable_feedback', 0)
        self.index_mcp_limits = get_user_config_var('Feedback.INDEX_MCP_LIMITS', (0.2, 0.4))
        self.feedback_state = False
        self.feedback_counter = 0
        self.feedback_timeout = 5
